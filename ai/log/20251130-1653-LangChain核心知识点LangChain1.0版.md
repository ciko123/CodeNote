# LangChain 核心知识点 LangChain 1.0+ 版

> **用途**：用于生成 Jupyter Notebook 教学文档
> 
> **原则**：最少最关键最有效的代码验证知识点
> 
> **版本**：基于 LangChain 1.0+ API
...
### Structured Output（LangChain 1.0+ 推荐方式）
**知识点说明**：通过 `llm.with_structured_output()` 方法，让 LLM 输出结构化格式，比传统 OutputParser 更简洁。
...
- 定义 Pydantic 模型：
  ...
  class Output(BaseModel):
      ...
      summary: str
  ```
- 使用 `llm.with_structured_output(Output)`
...
**知识点说明**：用于严格解析模型输出，使其强制符合结构化格式（兼容性用法）。
...
**知识点说明**：将 LCEL 链包装为支持消息历史的链，是 LangChain 1.0+ 的推荐方式。
...
**案例要求**：
- 创建基于 session_id 的历史工厂
- 配置 RunnableWithMessageHistory
- 验证不同会话的历史隔离
- 验证点：不同 session_id 的历史互不干扰
...
### Agent 创建（LangChain 1.0+ 完整流程）
**知识点说明**：使用 `create_react_agent` 创建 Agent，配合 `AgentExecutor` 执行。
...
**案例要求**：
- 使用 create_react_agent(llm, tools, prompt) 创建 Agent
...
### LCEL 配置管理
**知识点说明**：LangChain 1.0+ 的动态配置和热切换能力。
...
**案例要求**：
- 使用 chain.configurable_alternatives() 配置多个 LLM
- 演示运行时配置切换
- 验证点：能动态切换不同的模型配置
...
### **现代化 API**
- 优先使用 `with_structured_output()` 替代传统 OutputParser
...
1. **按优先级顺序**：先核心后高级
2. **独立验证**：每个知识点独立可运行
3. **现代化 API**：使用 LangChain 1.0+ 推荐方式
4. **完整示例**：提供从导入到验证的完整代码
5. **错误处理**：包含基础的异常处理示例

### **学习路径建议**
1. **Week 1**：掌握核心优先级内容（重点 LCEL 和消息历史）
...
### **从旧版本迁移**
- **Memory → RunnableWithMessageHistory**：消息历史管理升级
- **OutputParser → with_structured_output**：结构化输出简化
- **LLMChain → LCEL 语法**：链式调用现代化

### **兼容性说明**
- 传统 API 仍然可用，但建议使用新 API
- 新 API 提供更好的类型安全和性能
- 逐步迁移可减少学习成本

---

**🎯 目标**：基于 LangChain 1.0+ API，用最少的代码，最关键的知识点，最有效的验证方式，帮助学习者快速掌握现代化 LangChain 核心技能。
